...
public class AboutLocale {
    ...
    public void localizedOutputOfDates() {
        ...
        Locale localePL = new Locale("pl"); // Poland
        DateFormat dateformatPL = DateFormat.getDateInstance(DateFormat.FULL, localePL);
        // Well if you don't know how to type these characters, try "de", "it" or "us" ;-)
        assertEquals(dateformatPL.format(date), "niedziela, 3 kwietnia 2011");
    }

    @Koan
    public void getCountryInformation() {
        Locale locBR = new Locale("pt", "BR");
        assertEquals(locBR.getDisplayCountry(), "Brazylia");
        assertEquals(locBR.getDisplayCountry(locBR), "Brasil");

        Locale locCH = new Locale("it", "CH");
        assertEquals(locCH.getDisplayCountry(), "Szwajcaria");
